require "spec_helper"
require_relative "../../../../lib/laa_crime_forms_common/risk/nsm/v1/low_risk_detector"

RSpec.describe LaaCrimeFormsCommon::Risk::Nsm::V1::LowRiskDetector do
  ...
  let(:work_items) do
    [attendance_work_item, advocacy_work_item, preparation_work_item].map { _1.transform_keys(&:to_s) }
  end
  let(:attendance_work_item) { { work_type: "attendance_with_counsel", time_spent: attendance_time_spent } }
  let(:preparation_work_item) {  { work_type: "preparation", time_spent: preparation_time_spent } }
  let(:advocacy_work_item) {  { work_type: "advocacy", time_spent: advocacy_time_spent } }
